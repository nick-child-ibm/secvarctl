...
int performGenerateCommand(int argc,char* argv[])
{
	...
	//argp will only allow one string in argv as the program name, we want it to be 'secvarctl generate' 
	...
	{
		{"var_name", 'n', "VAR_NAME", 0 , "name of a secure boot variable, used when generating an PKCS7/Auth file."
										" Also, when an ESL or Auth file contains hashed data use '-n dbx'."
										" currently accepted values: {'PK','KEK','db','dbx'}"},
		{"hash_alg", 'h', "HASH_ALG", 0, "hash function, use when '[h]ash' is input/output format."
		...
		{"external_sig", 's', "FILE", 0, "raw signed data, alternative to using private keys when generating PKCS7/Auth"
...
static int getOutputData(const unsigned char *buff, size_t size, struct Arguments *args, const struct hash_funct *hashFunction, unsigned char **outBuff, size_t *outBuffSize) 
{
	...
	switch (args->outForm[0]) {
		...
		case 'e':
			...
			break;
		default:
			prlog(PR_ERR, "ERROR: Unknown output format %s\n", args->outForm);
			rc = ARG_PARSE_FAIL;
	}
out:
	return rc;
