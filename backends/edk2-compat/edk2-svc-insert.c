...
int performInsertCommand(int argc, char *argv[])
{
	...
	// 1. get new ESL
	new_esl_buff = (unsigned char *)getDataFromFile(args.new_esl, &new_esl_size);
	...
	if (args.input_method == READ_FROM_SECVAR_PATH)
		...
		current_esl_buff =
			(unsigned char *)getDataFromFile(args.current_esl, &curr_esl_size);
